...
class Entity(base.CrosslineBase):
    ...
    identifier = appier.field(index="hashed", default=True, immutable=True)

    key = appier.field(index="hashed", immutable=True)
    ...
    @classmethod
    @appier.operation(
        name="Create", parameters=(("Identifier", "identifier", str),), factory=True
    )
    ...
    @appier.operation(
        ...
        parameters=(("Code", "code", int), ("Secret", "secret", int)),
    )
