...
class PicaAdapter(base.BaseAdapter):
    ...
    def enter(self, info=None, app=None, *args, **kwargs):
        ...
        if not movement in ("Duplicado",):
            info["pica:movimento"] = movement
        ...
        appier.post(
            ...
            params=dict(
                empresa_id=company, codigo=code, senha=secret, tipo_movimento=movement
            ),
        )
    ...
    def _res_movement(self, entity, timestamp=None):
        ...
        if not latest:
            return "Entrada"
        if timestamp - latest.timestamp < cls.DUPLICATE_THRESHOLD:
            return "Duplicado"
        if timestamp - latest.timestamp > cls.TURN_THRESHOLD:
            return "Entrada"
